...
public OnGameModeExit()
{
    ...
    for (new i = 0, j = GetMaxPlayers(); i <= j; i++) // GetPlayerPoolSize function was added in 0.3.7 version and gets the highest playerid currently in use on the server
    {
...
Dialog:Login(playerid, response, listitem, inputtext[])
{
    if (response)
    {
        ...
        if (strcmp(hashed_pass, Player[playerid][Password]) == 0)
        {
            ...
            SetSpawnInfo(playerid, NO_TEAM, 0, Player[playerid][X_Pos], Player[playerid][Y_Pos], Player[playerid][Z_Pos], Player[playerid][A_Pos]);
